...
class Matrix( object ):
	...
	def get(self, row, col):
		try:
			...
			if row >= self.rows:
				raise IndexError("Received a row index greater than the rows contained (indices are zero-based): %i > %i" % (row, self.rows - 1))
			elif col >= self.cols:
				raise IndexError("Received a column index greater than the columns contained (indices are zero-based): %i > %i" % (col, self.cols - 1))
			else:
	...
	def set(self, row, col, value):
		try:
			...
			if row >= self.rows:
				raise IndexError( "Received a row index greater than rows contained (indices are zero-based): %i > %i" % ( row, self.rows ) )
			elif col >= self.cols:
				raise IndexError( "Received a column index greater than the columns contained (indices are zero-based): %i > %i" % ( col, self.cols ) )
			else:
				raise IndexError( "No element at row %i and col %i" % ( row, col ) )
	...
	def set_row(self, index, elements):
		try:
			...
			if len(elements) < self.cols - 1:
				raise IndexError( "The total elements received is not equal to the total columns on the matrix. Exiting..." )
			else:
				raise ie
